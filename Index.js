...
// Helper to build a stream object that opens in browser
...
async function movieStreams(imdbId) {
  const streams = [
    ...
    makeStream('VidSrc2', '▶ Watch on VidSrc (Mirror)', `https://vidsrc.cc/v2/embed/movie/${imdbId}`),
  ];
...
async function seriesStreams(fullId) {
  ...
  const streams = [
    ...
    makeStream('VidSrc2', `▶ VidSrc Mirror  S${season}E${episode}`, `https://vidsrc.cc/v2/embed/tv/${imdbId}/${season}/${episode}`),
  ];
