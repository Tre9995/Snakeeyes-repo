...
const MANIFEST = {
  ...
  description: 'Stream movies and TV shows via VidSrc, VidLink, Videasy, EmbedSu, SuperEmbed, Autoembed, SmashyStream and VidBinge.',
  types: ['movie', 'series'],
  resources: [
...
async function movieStreams(imdbId) {
  ...
  const streams = [
    ...
    },
    {
      name: 'Snakeeyes',
      title: '▶ SmashyStream',
      externalUrl: `https://player.smashy.stream/movie/${imdbId}`
    }
  ];
...
async function seriesStreams(fullId) {
  ...
  const streams = [
    ...
    {
      name: 'Snakeeyes',
      title: `▶ EmbedSu S${season}E${episode}`,
    ...
    {
      name: 'Snakeeyes',
      title: `▶ SmashyStream S${season}E${episode}`,
      externalUrl: `https://player.smashy.stream/tv/${imdbId}?s=${season}&e=${episode}`
    }
  ];
